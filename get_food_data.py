...
def get_data():
    with open('food_index_dg_suseong.txt', 'r', encoding='utf-8') as f:
        data_index=f.readlines()
    with open('food_menu_dg_suseong.txt', 'r', encoding='utf-8') as f:
        data_menu = f.readlines()
    data_index=[d.replace('\n','') for d in data_index]
    data_index=[d.split(",") for d in data_index]
    #퓨전/뷔페 6 디저트/베이커리 10 한식 88 일식 11 전통차/커피전문점 7 양식 6 세계요리 8 중식 6
    #categroy :fusion-beffet  desert-bakery ,korean ,japanese ,tea-coffe ,western ,global ,chinese
    korean=['퓨전/뷔페','디저트/베이커리','한식','일식','전통차/커피전문점','양식' ,'세계요리','중식']
    english=['fusion-beffet',  'desert-bakery' ,'korean' ,'japanese' ,'tea-coffe' ,'western' ,'global' ,'chinese']
